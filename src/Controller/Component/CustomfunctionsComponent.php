<?php
	...
	class CustomfunctionsComponent extends Component {
		...
		public function checkFatSpreadOrBevo($customer_id) {
			...
			if ($this->checkCaBevo($customer_id) == 'yes') {
				...
				foreach ($sub_commodities_array as $each_sub_commodity) {
					...
					if ($each_sub_commodity == '172')//changed fron 173 to 172 on 11-07-2019 by Amol
					{
						$fat_spread = 'yes';
					}
		...
		{ 
			...
			//pr($firm_details['customer_id']);die;
			...
			if(!empty($tableRowData)){
				...
				foreach($tableRowData as $each){ 
					...
					//.$each['tbl_name'].$each['printer_name'];
			...
			// if(count($resultdata) == 0){
				...
				//$data = "MECARD:N:".'Esigned By:'.$result[0].'(Chemist In-charge)'.";EMAIL:".$result[1]['firm_name']."|"."Commodity:".$commdity_names.";";
				
				//$data = "Commodity:".$each['commodity_name'].";";
				//$qrdata = $data1.$data2;
