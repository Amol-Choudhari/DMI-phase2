...
//on 20-01-2020 by Amol	
	...
	public function my_output($name='doc.pdf', $dest='I',$pdf_file=null,$cer_value=null,$pkcs7_value=null,$signing=false){
		...
		if ($this->sign) {
			...
			if($pdf_file != null){				
				...
				// remove last newline
					...
					$_SESSION['pdf_byte_range_1'] = $byte_range[1];//storing the value in session to use in next call to sign pdf.
			...
			if($cer_value != null){
				...
				$signature = str_pad($signature, $this->signature_max_length, '0');
				// Add signature to the document
				$this->buffer = substr($pdfdoc, 0, $_SESSION['pdf_byte_range_1']).'<'.$signature.'>'.substr($pdfdoc, $_SESSION['pdf_byte_range_1']);
